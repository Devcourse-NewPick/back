  ...
  async crawlSingleNews(browser: puppeteer.Browser, news: any): Promise<any> {
    ...
    try {
      ...
      const crawlingNews = await page.evaluate(() => {
        ...
        const dateTime = document.querySelector('.media_end_head_info_datestamp_time')?.getAttribute('data-date-time') || null;
        ...
        const category = document.querySelector('.media_end_categorize_item')?.textContent?.trim() || news.category || null;
